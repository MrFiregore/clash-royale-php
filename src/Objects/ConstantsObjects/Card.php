    ...
    class Card extends BaseObject
    {
        ...
        public function getConstant ()
        {
            if (is_null(self::$constants_stats)) {
                self::$constants_stats = collect(CRConstant::getConstant('cards_stats'))->transform(
                    function ($item, $key) {
            ...
            if (is_null($this->constant)) {
                $stats                       =
                    collect(self::$constants_stats->get(strtolower($this->getType())))
                        ->where('name', $this->getKey())
                        ->values()
                        ->all();
                $this->constant['stats']     = (empty($stats)) ? $stats : $stats[0];
                $constants                   =
                    self::$constants_rarities->where('name', $this->getRarity())
                                             ->values()
                                             ->all();
